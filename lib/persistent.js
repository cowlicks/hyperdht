...
module.exports = class Persistent {
  ...
  verifyAnnounce({ value, target, token, }, nodeId) {
    const ann = decode(m.announce, value);
    const signable = annSignable(target, token, nodeId, ann, NS.ANNOUNCE)
    const { peer, signature } = ann
    return sodium.crypto_sign_verify_detached(signature, signable, peer.publicKey);
  }
...
/// create a 64 byte buffer for being signed.
/// Write NAMESPACE bytes to first part to identify the kind of buffer
/// hash arguments (target, id, token, peer, etc) and write hash to second part of buffer
function annSignable (target, token, id, ann, ns) {
  ...
  // write NS to first half of buffer
  signable.set(ns, 0)

  // let concatted_buf = concats target, id, token and encoded ann;
